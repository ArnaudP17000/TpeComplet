...
class TPE:
    ...
    def __post_init__(self):
        ...
        for carte in self.cartes_commercant:
            if not isinstance(carte, int) or carte < 0:
    ...
    def to_dict(self):
        ...
        return {
            ...
            'cartes_commercant': self.cartes_commercant,
    ...
    def from_dict(cls, data):
        ...
        # Compatibilité avec anciennes versions (carte_commercant -> cartes_commercant)
        cartes = data.get('cartes_commercant')
        if cartes is None:
            # Ancienne version avec carte_commercant unique
            carte_unique = data.get('carte_commercant')
            cartes = [carte_unique] if carte_unique else []
    ...
    def exporter_excel(self, nom_fichier: str = "tpe_export.xlsx") -> bool:
        ...
        try:
            ...
            headers = [
                ...
                "Régisseurs Suppléants", "Cartes Commerçant", "ShopID", "Nombre de TPE",
            ...
            for row_idx, tpe in enumerate(self.tpes, start=2):
                ...
                ws.cell(row=row_idx, column=6, value=", ".join(map(str, tpe.cartes_commercant)))
                ws.cell(row=row_idx, column=7, value=tpe.shop_id)
                ws.cell(row=row_idx, column=8, value=tpe.nombre_tpe)
                ws.cell(row=row_idx, column=9, value="Oui" if tpe.acces_backoffice.actif else "Non")
                ws.cell(row=row_idx, column=10, value=tpe.acces_backoffice.email or "")
                ws.cell(row=row_idx, column=11, value=tpe.modele_tpe)
                ws.cell(row=row_idx, column=12, value="Oui" if tpe.type_tpe.ethernet else "Non")
                ws.cell(row=row_idx, column=13, value="Oui" if tpe.type_tpe.quatre_cinq_g else "Non")
                
                if tpe.type_tpe.config_reseau:
                    ws.cell(row=row_idx, column=14, value=tpe.type_tpe.config_reseau.adresse_ip)
                    ws.cell(row=row_idx, column=15, value=tpe.type_tpe.config_reseau.masque)
                    ws.cell(row=row_idx, column=16, value=tpe.type_tpe.config_reseau.passerelle)
                
                ws.cell(row=row_idx, column=17, value=tpe.date_creation)
